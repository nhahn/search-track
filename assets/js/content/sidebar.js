...
listApp.controller('RootCtrl', function ($scope, $dexieBind) {
  ...
  $scope.tasks = [];
  // TODO organize the controllers better so they don't get in each other's way
  setTimeout(function() {
    $scope.$apply(function() {
      $dexieBind.bind(db, db.Task.limit(1000), $scope).then(function(data) {
        console.log (data);
        $scope.tasks = data.sort(function(a,b) {
          return b.dateCreated - a.dateCreated;  
        }).filter(function(a) {return !a.name.includes("Unknown")});
        $scope.$broadcast('tasksloaded');
      });

      // TODO add pages associated with the task
    });
  }, 50);
